  ...
  parseConfigResponse(lines) {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('A') && line.includes('[')) {
        console.error('DEBUG: Analog line:', line);
      }
      const analogMatch = line.match(/^A(\d)=\s+(Port|Stbd)\s+(.+?)\s*\[([+\-]\d+),([+\-]\d+),(.)\]\s+([0-9A-F]+)/);
      if (analogMatch) {
        console.error('DEBUG: Regex MATCHED for', analogMatch[1]);
        ...
        console.error('DEBUG: Regex FAILED to match analog line');
        
        // Fallback: Try old parsing logic without calibration values
        const fallbackMatch = line.match(/^A(\d)=\s+(Port|Stbd)\s+(.+?)\s+\[/);
        if (fallbackMatch) {
          console.error('DEBUG: Fallback matched, adding without calibration');
